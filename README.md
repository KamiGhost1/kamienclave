...
Поверх лицензионного сервера появился отдельный **control plane** — веб-панель,
которая управляет всем жизненным циклом доставки: арендаторы и роли, загрузка
сборок, **build-пайплайн** (компиляция bytecode / обфускация bundle) со статусами
как в CI/CD, версионирование и публикация, выпуск лицензий с привязкой к аккаунту
и лицензирование дочерних серверов.

**Новый подход — что изменилось:**

- **Build-пайплайн вместо ручной сборки.** Каждая загруженная сборка проходит
  очередь со стадиями `validate → compile|obfuscate → publish`. Видно состояние
  каждой стадии, логи и ошибки — как в CI/CD-инструментах.
- **Версионирование и публикация.** У сборки много версий; публикация помечает
  «живую» версию. Можно собрать сейчас, а **выложить позже** (held) — вручную или
  автоматически по флагу.
- **Лицензии привязаны к аккаунтам + роли.** `owner` выпускает лицензии и
  привязывает их к пользователю; `admin` оперирует (revoke/activate, грузит
  сборки); обычный пользователь (`viewer`) видит **только свои** лицензии.
- **Крипто остаётся в Go.** Запечатывание `*.lic`, `.encpkg`, подпись серверных
  лицензий и компиляция bytecode выполняются Go-сервисом (`mint-service`); панель
  только оркестрирует. См. TECHNICAL §7.1 (компилятор/watermark не попадают к клиенту).

> Сам control plane (NestJS + React SPA) и Go-`mint-service` живут в отдельных
> репозиториях; здесь — обзор подхода и скриншоты.

### Build-пайплайн (стадии, статусы, логи, публикация)

![Pipeline](docs/screenshots/pipeline.png)

### Обзор флота и активности

![Overview](docs/screenshots/overview.png)

### Лицензии (выпуск, привязка к аккаунту, скачивание `.lic`)

![Licenses](docs/screenshots/licenses.png)

### Загрузка сборок

![Builds](docs/screenshots/builds.png)
